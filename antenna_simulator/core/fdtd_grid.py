...
from .geometry import Vector3D, BoundingBox, AntennaGraph, GeometryPrimitive
...
class FDTDGrid:
            ...
            for j in range(j_min, j_max + 1):
                for k in range(k_min, k_max + 1):
                    ...
                    if geometry.contains_point(cell_center):
                        self.apply_material(
                            geometry.material,
                            (slice(i, i + 1), slice(j, j + 1), slice(k, k + 1))
                        )
    ...
    def setup_pml(self, pml_config: PMLMaterial = None):
        ...
        if pml_config is None:
            pml_config = PMLMaterial(
                thickness=self.config.pml_layers,
                order=3
            )
            pml_config.sigma_max = pml_config.calculate_optimal_sigma(
                self.config.dx
            )
    ...
    def get_energy(self) -> Tuple[float, float]:
        ...
        H_energy = 0.5 * dV * (
            np.sum(self.Hx ** 2) * MU_0 +
            np.sum(self.Hy ** 2) * MU_0 +
            np.sum(self.Hz ** 2) * MU_0
        )
