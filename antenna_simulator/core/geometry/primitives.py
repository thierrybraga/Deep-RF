...
class Rectangle(GeometryPrimitive):
    ...
    def contains_point(self, point: Vector3D) -> bool:
        hw, hh, ht = self.width/2, self.height/2, self.thickness/2
        return (abs(point.x - self.center.x) <= hw and
                abs(point.y - self.center.y) <= hh and
                abs(point.z - self.center.z) <= ht)
...
class Horn(GeometryPrimitive):
    ...
    def get_bounding_box(self) -> BoundingBox:
        ...
        return BoundingBox(
            ...
            Vector3D(self.center.x + max_w/2, self.center.y + max_h/2, self.center.z + self.length)
        )
    ...
    def contains_point(self, point: Vector3D) -> bool:
        ...
        in_outer = (abs(point.x - self.center.x) <= curr_w/2 + self.wall_thickness and
                    abs(point.y - self.center.y) <= curr_h/2 + self.wall_thickness)
                    
        in_inner = (abs(point.x - self.center.x) <= curr_w/2 - self.wall_thickness and
                    abs(point.y - self.center.y) <= curr_h/2 - self.wall_thickness)
