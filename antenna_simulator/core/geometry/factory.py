...
from .primitives import Vector3D, Wire, Rectangle, Helix, Horn, ParabolicDish
...
class AntennaFactory:
    ...
    def create_dipole(
        ...
        n_center, n_top, e1 = antenna.add_wire(
            ...
            start_type="feed", end_type="terminal"
        )
    ...
    def create_patch(
        ...
        delta_L = substrate_h * 0.412 * (
            (eps_eff + 0.3) * (width / substrate_h + 0.264)
        ...
        feed_wire = Wire(
            start=Vector3D(feed_x, 0, 0),
            end=Vector3D(feed_x, 0, substrate_h),
            radius=0.5e-3
        )
        feed_wire.material = MaterialLibrary.COPPER
        
        n_ground, n_patch, _ = antenna.add_wire(
            feed_wire.start, feed_wire.end,
            radius=0.5e-3,
            start_type="ground",
            end_type="feed"
        )
        ...
        for i in range(n_elements):
            ...
            if prev_node_top is not None:
                ...
                antenna.add_wire(
                    ...
                    wire_radius,
                    start_type="junction", end_type="junction"
                )
