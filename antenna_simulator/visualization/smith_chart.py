...
def calculate_impedance_from_fdtd(
    ...
    impedance = np.where(np.real(impedance) < 0, 
                         -np.real(impedance) + 1j*np.imag(impedance),
                         impedance)
...
def calculate_impedance_analytical(
    ...
    **kwargs
    ...
    if antenna_type.lower() == 'dipole':
        ...
        dipole_res = calculate_impedance_analytical(
            'dipole', frequencies, length, z0
        )
        ...
        for i, freq in enumerate(frequencies):
            ...
            dipole_z = calculate_impedance_analytical('dipole', np.array([freq]), length, z0).impedance[0]
...
class SmithChart:
    ...
    def _draw_chart(self):
        ...
        if self.config.show_grid:
            ...
            # Eixo real (X = 0)
            ax.axhline(y=0, color=self.config.grid_color, 
    ...
    def plot_trace(
        ...
        if marker_freq is not None and frequencies is not None:
            for freq in marker_freq:
                ...
                self.ax.scatter(
                    gamma.real, gamma.imag,
                    marker='o', c=color, s=50, zorder=10
                )
...
def plot_s11_vs_frequency(
    ...
    best = result.find_best_match()
    ax.scatter(
        best['frequency']/1e6, best['s11_db'],
        marker='*', c='gold', s=200, zorder=10,
    ...
    if bw[2] > 0:
        ax.axvspan(bw[0]/1e6, bw[1]/1e6, alpha=0.2, color='green',
...
def plot_impedance_vs_frequency(
    ...
    ax1.axhline(y=result.z0, color='r', linestyle='--', alpha=0.5, 
                label=f'{result.z0:.0f} Ω (referência)')
    ...
    ax2.scatter(
        res['frequency']/1e6, res['reactance'],
        marker='*', c='gold', s=200, zorder=10,
...
if __name__ == "__main__":
    ...
    result = calculate_impedance_analytical(
        'dipole',
        frequencies,
    ...
    chart.plot_result(
        ...
        label='Dipolo λ/2',
        color='blue',
        marker_freqs=[280e6, 300e6, 320e6]
    )
