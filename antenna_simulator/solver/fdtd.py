...
from core.grid import FDTDGrid
from .kernels import update_h_kernel, update_e_kernel
    ...
    def _update_H(self):
        ...
        if self.use_numba:
            update_h_kernel(
                ...
                self.grid._Da_x, self.grid._Db_x,
                self.grid._Da_y, self.grid._Db_y,
                self.grid._Da_z, self.grid._Db_z,
                self.grid.config.dx, self.grid.config.dy, self.grid.config.dz
            )
        else:
    ...
    def _update_E(self):
        ...
        if self.use_numba:
            update_e_kernel(
                self.grid.Ex, self.grid.Ey, self.grid.Ez,
                self.grid.Hx, self.grid.Hy, self.grid.Hz,
                self.grid._Ca_x, self.grid._Cb_x,
                self.grid._Ca_y, self.grid._Cb_y,
                self.grid._Ca_z, self.grid._Cb_z,
                self.grid.config.dx, self.grid.config.dy, self.grid.config.dz
            )
        else:
            dx, dy, dz = self.grid.config.dx, self.grid.config.dy, self.grid.config.dz
    ...
    def _apply_sources(self):
        ...
        for source in self.sources:
            if source.is_active(self.current_time):
                ...
                # Verifica limites
                if (0 <= i < field_array.shape[0] and 
                    0 <= j < field_array.shape[1] and 
                    0 <= k < field_array.shape[2]):
