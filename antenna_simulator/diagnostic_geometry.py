...
from antenna_simulator.core.geometry.factory import AntennaFactory
from antenna_simulator.core.geometry.primitives import Vector3D
from antenna_simulator.core.fdtd_grid import create_grid_for_antenna
from antenna_simulator.fem.mesh_generator import MeshGenerator
...
def run_diagnostic():
    ...
    cases = [
        ...
        ("Off-Center Dipole", lambda: AntennaFactory.create_dipole(length=0.1, center=Vector3D(0.05, 0.05, 0))),
    ...
    for name, factory_func in cases:
        ...
        freq = 1e9
        try:
            grid = create_grid_for_antenna(antenna, freq_max=freq, cells_per_lambda=10, pml_layers=4)
            ...
            center_world = grid.grid_to_world(grid.config.nx//2, grid.config.ny//2, grid.config.nz//2)
            ...
            print(f"FDTD Feed Recovered: {print_vec(feed_recovered)} (Error: {dist_error*1000:.2f} mm)")
