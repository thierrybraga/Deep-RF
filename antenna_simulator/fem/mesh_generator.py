...
try:
    ...
    from antenna_simulator.core.geometry.primitives import Wire, Rectangle, Helix, Horn, ParabolicDish, Cylinder
except ImportError:
    # Fallback para execução local ou testes
    try:
        from core.geometry.topology import AntennaGraph
        from core.geometry.primitives import Wire, Rectangle, Helix, Horn, ParabolicDish, Cylinder
    except ImportError:
        # Tenta relativo
        from ..core.geometry.topology import AntennaGraph
        from ..core.geometry.primitives import Wire, Rectangle, Helix, Horn, ParabolicDish, Cylinder
...
class MeshGenerator:
    ...
    def generate_2d(self, filename: str = None, plane: str = 'xz', resolution_factor: float = 10.0):
        ...
        for dim, tag in all_curves:
            ...
            is_boundary = (
                abs(cx_min - u_min) < eps or abs(cx_max - u_max) < eps or
                abs(cy_min - v_min) < eps or abs(cy_max - v_max) < eps
            )
    ...
    def generate(self, filename: str = None, resolution_factor: float = 10.0):
        ...
        air_box = gmsh.model.occ.addBox(domain_min.x, domain_min.y, domain_min.z, 
        ...
        for edge_id, edge in self.antenna.edges.items():
            ...
            if isinstance(geo, Wire):
                ...
                cyl = gmsh.model.occ.addCylinder(geo.start.x, geo.start.y, geo.start.z,
                                                 dx, dy, dz, r)
                ...
                rect = gmsh.model.occ.addBox(corner.x, corner.y, corner.z,
                                             geo.width, geo.height, thick)
        ...
        if antenna_tags:
            ...
            res, _ = gmsh.model.occ.cut([(3, air_box)], antenna_tags, removeObject=True, removeTool=True)
        ...
        air = gmsh.model.occ.addBox(domain_min.x, domain_min.y, domain_min.z, 
                                    box_dim.x, box_dim.y, box_dim.z)
        ...
        for c in all_curves:
            ...
            tol = margin * 0.1
            if (cx > domain_min.x + tol and cx < domain_max.x - tol and
                cy > domain_min.y + tol and cy < domain_max.y - tol and
                cz > domain_min.z + tol and cz < domain_max.z - tol):
