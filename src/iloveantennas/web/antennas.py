import numpy as np
from iloveantennas.web.config import AntennaConfig
...
def create_antenna(config: AntennaConfig):
    ...
    if config.type == "dipole":
        ...
        length = config.length or wavelength / 2
        return AntennaFactory.create_v_dipole(length=length, radius=config.radius)

    elif config.type == "folded_dipole":
        length = config.length or wavelength / 2
        spacing = length * 0.02
        return AntennaFactory.create_folded_dipole(
            length=length, radius=config.radius, spacing=spacing
        )
        ...
        return AntennaFactory.create_monopole(
            length=length, radius=config.radius, ground_size=ground_size
        )

    elif config.type == "yagi":
        return AntennaFactory.create_yagi(
            frequency=config.frequency, num_directors=config.num_directors
        )
        ...
        return AntennaFactory.create_helix(frequency=config.frequency, num_turns=config.turns)
        ...
        radius = config.loop_radius or wavelength / (
            2 * np.pi
        )  # 1 comprimento de onda de circunferência
        return AntennaFactory.create_loop(radius=radius, wire_radius=config.radius)
...
def get_antenna_geometry_data(antenna) -> dict:
    ...
    return {
        ...
        "feed_point": (
            [antenna.feed_point.x, antenna.feed_point.y, antenna.feed_point.z]
