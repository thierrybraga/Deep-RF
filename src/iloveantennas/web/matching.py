...
def _virtual_resistor_and_q(
    z0: float, z_load: complex, q_target: float | None
...
def _l_section_from_resistors(
    r_series_side: float, r_shunt_side: float, q_section: float, f: float
) -> tuple[float, float]:
    w = 2 * math.pi * f
...
def synthesize_pi_network(
    z0: float, z_load: complex, f: float, q_target: float | None
) -> MatchingResult:
    notes: list[str] = []
    if z_load.real <= 0:
    ...
    if q_low <= 0:
        q_low = q_high
        notes.append(
            "Q da seção de baixa resistência ajustado para Q da seção de alta resistência."
        )
...
def calculate_matching(
    z_load: complex, z0: float, f: float, network_type: NetworkType, q: float | None
) -> MatchingResult:
    if network_type == "L":
