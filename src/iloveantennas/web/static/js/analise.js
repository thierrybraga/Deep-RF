class AnalisePage {
    ...
    async loadAnalysis() {
        ...
        const [smithRes, radRes] = await Promise.all(requests);
        const [smithJson, radJson] = await Promise.all([smithRes.json(), radRes.json()]);
        ...
        if (radJson.success) {
            ...
            this.chartManager.initRadiationChart('analise-rad-h-canvas');
