...
from iloveantennas.simulator.core.constants import C0, MaterialLibrary
from iloveantennas.simulator.core.geometry import AntennaFactory, Vector3D
from iloveantennas.simulator.core.grid import FDTDGrid, GridConfig, create_grid_for_antenna
from iloveantennas.simulator.solver import (
    FDTDSolver,
    FieldProbe,
    GaussianSource,
...
from iloveantennas.simulator.visualization.smith_chart import (
    ...
    plot_s11_vs_frequency,
    plot_vswr_vs_frequency,
)
...
def run_dipole_example():
    ...
    config = GridConfig(
        ...
        nx=40,
        ny=40,
        nz=60,  # Grade menor para demonstração
        pml_layers=8,
        courant=0.99,
    )
    ...
    print(
        f"   Domínio: {config.domain_size[0]*100:.1f}×{config.domain_size[1]*100:.1f}×{config.domain_size[2]*100:.1f} cm"
    )
    ...
    tau = 2e-9  # 2 ns
    source = GaussianSource(position=center, component="Ez", amplitude=1.0, tau=tau)
    ...
    try:
        ...
        from iloveantennas.simulator.visualization.plots import (
            FieldVisualizer,
        ...
        viz = FieldVisualizer()
        if len(times) > 0:
            fig, ax = viz.plot_time_series(times, values, title="Campo Ez no ponto de alimentação")
        ...
        fig, ax = viz.plot_field_slice(
            field_slice, config.dx, config.dz, title="Campo Ez - Plano XZ"
        )
...
def run_yagi_example():
    ...
    yagi = AntennaFactory.create_yagi(frequency=freq, num_directors=3, boom_radius=0.01)
...
def run_patch_example():
    ...
    patch = AntennaFactory.create_patch(
        frequency=freq, substrate_er=4.4, substrate_h=1.6e-3  # FR4  # 1.6 mm
    )
...
def run_helix_example():
    ...
    helix = AntennaFactory.create_helix(frequency=freq, turns=5, mode="axial")
...
def run_smith_chart_example():
    ...
    result_dipole = calculate_impedance_analytical(
        "dipole", frequencies, length=dipole_length, z0=z0
    )
    ...
    result_monopole = calculate_impedance_analytical(
        "monopole", frequencies, length=monopole_length, z0=z0
    )
    ...
    chart.plot_impedance(complex(50, 0), label="Z₀ = 50 Ω", marker="*", color="gold", size=200)
    ...
    # S11 vs Frequência
    fig, ax = plot_s11_vs_frequency(result_dipole, title="S11 - Dipolo de Meia Onda")
    ...
    fig, axes = plot_impedance_vs_frequency(result_dipole, title="Impedância - Dipolo λ/2")
    ...
    # VSWR vs Frequência
    fig, ax = plot_vswr_vs_frequency(result_dipole, title="VSWR - Dipolo de Meia Onda")
...
Exemplos de uso:
  ...
  python main.py --materials         Lista materiais disponíveis
    ...
    parser.add_argument("--gui", action="store_true", help="Inicia interface gráfica (padrão)")
    parser.add_argument("--cli", action="store_true", help="Modo linha de comando interativo")
    parser.add_argument(
        "--example",
        type=str,
    ...
    parser.add_argument("--materials", action="store_true", help="Lista materiais disponíveis")
