import numpy as np
from skfem import *
from skfem.helpers import cross, curl, dot
...
class FEMSolver3D:
    ...
    def __init__(self, mesh_file: str, frequency: float, feed_point: Vector3D = None):
        if meshio is None:
            raise ImportError(
                "meshio is required for FEMSolver3D. Install it with 'pip install meshio'."
    ...
    def solve(self):
        ...
        k_complex = self.k0 * (
            1 - 0.1j
        )  # Perda fictícia para evitar ressonância infinita e simular radiação
        L = A - (k_complex**2) * M
    ...
    def get_fields_on_grid(self, nx=30, ny=30, nz=30):
        ...
        # Calculate magnitude
        mag = np.sqrt(
            np.abs(field_grid[0]) ** 2 + np.abs(field_grid[1]) ** 2 + np.abs(field_grid[2]) ** 2
        )
