...
from .primitives import BoundingBox, GeometryPrimitive, Rectangle, Vector3D, Wire
...
class AntennaGraph:
    ...
    def add_node(
        self, position: Vector3D, node_type: str = "junction", properties: dict = None
    ) -> int:
        ...
        node = AntennaNode(
            id=node_id, position=position, node_type=node_type, properties=properties or {}
        )
    ...
    def add_edge(
        ...
        length = (self.nodes[end_node].position - self.nodes[start_node].position).magnitude
    ...
    def _find_or_create_node(
        self, position: Vector3D, node_type: str, tolerance: float = 1e-6
    ) -> int:
    ...
    def load(cls, filepath: str) -> "AntennaGraph":
        ...
        for node_data in data["nodes"].values():
            graph.add_node(
                Vector3D(*node_data["position"]), node_data["type"], node_data.get("properties", {})
            )
        ...
        for edge_data in data["edges"].values():
            ...
            graph.add_edge(edge_data["start"], edge_data["end"], geometry)
