...
class GeometryVisualizer:
    ...
    def draw_rectangle(
        ...
        poly = Poly3DCollection(
            [vertices], alpha=alpha, facecolor=color, edgecolor="black", linewidth=1
        )
    ...
    def draw_antenna_graph(self, antenna, show_nodes: bool = True, show_labels: bool = True):
        ...
        """
        from iloveantennas.simulator.core.geometry import Cylinder, Helix, Rectangle, Wire

        # Desenha geometrias
        for geom in antenna.geometries:
            if isinstance(geom, Wire):
                self.draw_wire(
                    geom.start.to_tuple(), geom.end.to_tuple(), geom.radius, color="peru"
                )
            elif isinstance(geom, Rectangle):
                self.draw_rectangle(geom.center.to_tuple(), geom.width, geom.height, color="orange")
            elif isinstance(geom, Helix):
                self.draw_helix(
                    geom.center.to_tuple(), geom.radius, geom.pitch, geom.turns, geom.wire_radius
                )
            elif isinstance(geom, Cylinder):
                self.draw_cylinder(geom.center.to_tuple(), geom.radius, geom.height)
        ...
        if show_nodes:
            for node_id, node in antenna.nodes.items():
                ...
                # Cores por tipo
                colors = {"feed": "red", "ground": "green", "junction": "blue", "terminal": "gray"}
                color = colors.get(node.node_type, "black")
                marker_size = 100 if node.node_type == "feed" else 50

                self.ax.scatter(pos.x, pos.y, pos.z, c=color, s=marker_size, marker="o")
...
class FieldVisualizer:
    ...
    def plot_field_magnitude(
        ...
        extent = [0, slice_data.shape[0] * dx * 1000, 0, slice_data.shape[1] * dx * 1000]

        im = self.ax.imshow(slice_data.T, origin="lower", extent=extent, cmap="hot", aspect="auto")
    ...
    def plot_spectrum(
        self, times: np.ndarray, values: np.ndarray, title: str = "Espectro de Frequência"
    ):
...
class RadiationPatternPlot:
    ...
    def plot_3d(
        ...
        self.ax.plot_surface(
            X, Y, Z, facecolors=colors, alpha=0.8, rstride=1, cstride=1, antialiased=True
        )
...
class FieldAnimator:
    ...
    def create_animation(self, interval: int = 50, title: str = "Propagação do Campo"):
        """
        ...
        extent = [0, self.frames[0].shape[0] * dx, 0, self.frames[0].shape[1] * dx]
