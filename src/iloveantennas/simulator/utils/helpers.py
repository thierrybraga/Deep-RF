...
def calculate_link_budget(
    pt_dbm: float, gt_dbi: float, gr_dbi: float, freq: float, distance: float, losses_db: float = 0
) -> float:
    """
...
def interpolate_pattern(
    angles: np.ndarray, pattern: np.ndarray, new_angles: np.ndarray
) -> np.ndarray:
    """
...
def export_to_csv(filename: str, data: Dict[str, np.ndarray], header: str = None) -> None:
    """
...
def export_to_nec2(
    filename: str, wires: List[Dict], frequency: float, comments: str = None
) -> None:
    """
...
def save_field_vtk(
    filename: str, grid_shape: Tuple[int, int, int], dx: float, fields: Dict[str, np.ndarray]
) -> None:
    """
...
def load_nec2_output(filename: str) -> Dict[str, Any]:
    ...
    """
    results = {"impedance": [], "gain": [], "pattern": {"theta": [], "phi": [], "gain": []}}
