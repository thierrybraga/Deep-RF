...
try:
    ...
    from PyQt6.QtGui import QAction, QColor, QIcon, QKeySequence, QPainter
...
class SimulationWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            # Resultados finais
            results = {"total_steps": step + 1, "grid": self.grid, "solver": self.solver}
...
class GeometryCanvas(FigureCanvas if HAS_MATPLOTLIB else QWidget):
    ...
    def draw_antenna(self, antenna_graph):
        ...
        node_colors = {"feed": "red", "ground": "green", "junction": "blue", "terminal": "orange"}
...
class FieldCanvas(FigureCanvas if HAS_MATPLOTLIB else QWidget):
    ...
    def plot_field_slice(
        self, field: np.ndarray, plane: str = "xy", slice_index: int = None, title: str = "Campo"
    ):
        ...
        im = self.ax.imshow(
            data, origin="lower", cmap="RdBu_r", vmin=vmin, vmax=vmax, aspect="auto"
        )
...
class MainWindow(QMainWindow):
    ...
    def _create_preset(self, preset_type: str):
        ...
        try:
            ...
            if preset_type == "dipole":
                ...
                self.antenna = AntennaFactory.create_patch(
                    length=wavelength / 2, width=wavelength / 2
                )
        ...
        try:
            ...
            if config.antenna_type == AntennaType.DIPOLE:
                self.antenna = AntennaFactory.create_dipole(
                    length=config.length, radius=config.radius
                )
                ...
                self.antenna = AntennaFactory.create_yagi(
                    wavelength=wavelength, num_directors=num_dirs
                )
                ...
                self.antenna = AntennaFactory.create_helix(
                    radius=config.length / 10, pitch=spacing, num_turns=num_turns
                )
            ...
            if feed_pos:
                ...
                if config.source_type == "gaussian":
                    source = GaussianSource(
                        position=grid_pos, amplitude=1.0, tau=1.0 / config.frequency
                    )
                else:
                    source = SineSource(
                        position=grid_pos, amplitude=1.0, frequency=config.frequency
                    )
    ...
    def _update_field_display(self):
        ...
        E_mag = np.sqrt(self.grid.Ex**2 + self.grid.Ey**2 + self.grid.Ez**2)

        plane = self.plane_combo.currentText().lower()
        self.field_canvas.plot_field_slice(
            E_mag, plane=plane, title="|E| - Magnitude do Campo Elétrico"
        )
