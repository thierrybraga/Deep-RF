...
def update_h_kernel(Hx, Hy, Hz, Ex, Ey, Ez, Da_x, Db_x, Da_y, Db_y, Da_z, Db_z, dx, dy, dz):
    ...
    for i in prange(nx):
        for j in range(ny - 1):
            for k in range(nz - 1):
                curl_e = (Ez[i, j + 1, k] - Ez[i, j, k]) / dy - (Ey[i, j, k + 1] - Ey[i, j, k]) / dz
    ...
    for i in prange(nx - 1):
        for j in range(ny):
            for k in range(nz - 1):
                curl_e = (Ex[i, j, k + 1] - Ex[i, j, k]) / dz - (Ez[i + 1, j, k] - Ez[i, j, k]) / dx
    ...
    for i in prange(nx - 1):
        for j in range(ny - 1):
            for k in range(nz):
                curl_e = (Ey[i + 1, j, k] - Ey[i, j, k]) / dx - (Ex[i, j + 1, k] - Ex[i, j, k]) / dy
                Hz[i, j, k] = Da_z[i, j, k] * Hz[i, j, k] - Db_z[i, j, k] * curl_e


@jit(nopython=True, parallel=True, fastmath=True, cache=True)
def update_e_kernel(Ex, Ey, Ez, Hx, Hy, Hz, Ca_x, Cb_x, Ca_y, Cb_y, Ca_z, Cb_z, dx, dy, dz):
    ...
    for i in prange(nx):
        for j in range(1, ny - 1):
            for k in range(1, nz - 1):
                curl_h = (Hz[i, j, k] - Hz[i, j - 1, k]) / dy - (Hy[i, j, k] - Hy[i, j, k - 1]) / dz
    ...
    for i in prange(1, nx - 1):
        for j in range(ny):
            for k in range(1, nz - 1):
                curl_h = (Hx[i, j, k] - Hx[i, j, k - 1]) / dz - (Hz[i, j, k] - Hz[i - 1, j, k]) / dx
    ...
    for i in prange(1, nx - 1):
        for j in range(1, ny - 1):
            for k in range(nz):
                curl_h = (Hy[i, j, k] - Hy[i - 1, j, k]) / dx - (Hx[i, j, k] - Hx[i, j - 1, k]) / dy
